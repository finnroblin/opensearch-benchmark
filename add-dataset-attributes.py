...
Example of script usage:
    ...
    create new hdf5 file with nested
    add-dataset-attributes.py --nested ~/dev/opensearch/k-NN/benchmarks/perf-tool/dataset/data.hdf5 ~/dev/opensearch/datasets/data-nested.hdf5

    create new hdf5 file with filters
    add-dataset-attributes.py --filters ~/dev/opensearch/k-NN/benchmarks/perf-tool/dataset/data.hdf5 ~/dev/opensearch/datasets/data-nested.hdf5
...
class _Dataset:
    def run(self, source_path, target_path) -> None:
        ...
        with h5py.File(source_path, "r") as in_file:
            ...
            out_file.create_dataset('parents', (len(parents),), data=parents) # TODO: actual numpy encoding for the integer.
...
def main(argv):
    # TODO: change datasets calculated depending on if nested or filters is passed in. 
